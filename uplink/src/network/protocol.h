...
inline size_t
WritePacket(uint8_t* buffer, PacketType type, uint8_t flags, const void* payload, uint16_t payloadLen)
{
	PacketHeader header;
...
inline const uint8_t* GetPayload(const uint8_t* packet) { return packet + sizeof(PacketHeader); }
...
struct TimeSyncPacket {
	...
	int month;
	int year;
	bool paused; // Is the server paused?
